...
camera_settings_t::camera_settings_t()
    ...
    , fovy_degrees(60.0)
    , min_fovy_degrees(2.0)
    , max_fovy_degrees(100.0)
    , initial_height(5.0)
    , move_speed(100.0)
    , rotate_speed(0.3)
    , zoom_power(25.0)
...
void camera_settings_t::read(CfgReader& cfg)
{
    ...
    cfg.getDouble(section, "FovY", fovy_degrees);
    cfg.getDouble(section, "FovYMin", min_fovy_degrees);
    cfg.getDouble(section, "FovYMax", max_fovy_degrees);
...
void camera_settings_t::print_in_journal() const
{
    ...
    journal->debug("    fovy: " + QString::number(fovy_degrees));
    journal->debug("    fovy_min: " + QString::number(min_fovy_degrees));
    journal->debug("    fovy_max: " + QString::number(max_fovy_degrees));
