...
Графики движения всех поездов в данном сценарии располагаются в папке со сценарием. Для размещения графика движения необходимо создать в папке сценария каталог timetable. Создадим сценарий, назвав его trains_134_135, и в главном файле main.lua напишем такую конфигурацию
...
В каталоге сценария создаем папку timetable - в ней будут храниться файлы графиков движения. Эти файлы имеют формат XML. 
...
Между тегами Config располагается описание графика движения, которой состоит из секций с именем Station. Каждая секция описывает станцию, или произвольный пункт на маршруте, через который должен проследовать поезд в заданное время. 
...
Для станции отправления, самой первой станции в графике, допускается не указывать время прибытия - по сценарию поезд уже будет находиться на станции, и симулятор считает его прибывшим в момент времени начала игры.
...
Виджет отображает текущее время, расстояние до ближайшей по ходу движения станции, а также перечень станций, с указанием времени прибытия и времени отправления по графику, а также фактического времени прибытия и фактического времени отправления.

Проследованные поездом станции обозначаются цветом: зеленым - в случае проследования по графику (+/- 2 минуты), оранжевым - в случае отклонения от графика на 3 минуты и более.
...
Хотя имя поезда может быть любым, мы подразумеваем под именем поезда его номер, а номер всегда соотвествует графику. Что будет, если мы переименуем поезд?
...
Напомню, что переименование поезда возможно тремя способами: из кабины локомотива (F8), из интерфейса карты (правой кнопкой по имени поезда -> "Переименовать"), а также по триггеру в сценарии с помощью action-функции actionRenameTrain().

Пример переименования поезда в процессе исполнения сценария мы не рассматривали. Рассмотрим в этот раз. Пусть поезд едет от станции А до станции В по графику поезда 134, который мы привели выше, а на станции В меняет номер на 136. Создадим еще один график для поезда 136, в котором, для простоты, время прибытия на станцию В будет таким же, как и в графике 134.
...
Функция actionRenameTrain() может применяться и в других типах триггеров [аналогично тому, как это описано в главе 4](advanced.md).

### 5.4 Создание попутных и встречных поездов

Для организации движения попутных и встречных поездов в сценарии (кроме поездов игроков), предусмотрен режим автоматического управления движением. Все поезда сценария моделируются физически, и, соответственно, для формирования встречных и попутных поездов используются те же принципы, что и для создания поезда игрока. Поезда можно формировать из всего представленного в игре подвижного состава.

Для использования режима автоматического управления в сценарии следует указать параметр auto для соотвествующего поезда

```
-- Включение режима "Автоведение"
train134.auto = true 
```

При запуске игры данный поезд автоматически приводится в рабочее состояние и переходит в режим автоматического ведения. При этом

* Рассчитывается скорость движения для соблюдения графика, если график задан.
* Определяется текущее ограничение скорости, с учетом снижения скорости при следующем более строгом ограничении
* Определяется скорость движения, с учетом сигнала светофора.
* Из вышеперечисленных скоростей выбирается наименьшая.
* Выполняется автоматическое управление тягой и торможением.

В любой момент режим автоведения может быть выключен нажатием клавиш Alt + F. Этот режим может быть включен в любой момент повторным нажатием клавиш Alt + F. 

Таким образом, все поезда, существующие в данной игровой сессии, имеют возможность управляться автоматически или в ручном режиме (игроком). Понятия трафик, в том смысле, который вкладывается в данный термин в других железнодорожных симуляторах, в RRS нет - все поезда, существующие в игре, абсолютно равноценны, и к ним применяются все имеющиеся игровые механики.