...
Пока поезд 104 доедет до второго приближения, маршрут уже будет построен. Соответственно горловина станции В будет знатяна и сквозной маршрут этому поезду построить не выйдет. Однако, создав проблему, мы героически решим её с помощью такой функции для позиционного триггера
...
мы ставим триггер на его совобождение, и уже этот триггер построит для поезда 104 маршрут пропуска, когда поезд 102 уберется с первого удаления
...
Оставляем триггер только в том случае, есле второй участок приближения еще не занимался
...
Не забываем установить нафу триггерную функцию в системе
