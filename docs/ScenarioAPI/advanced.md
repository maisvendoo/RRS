...
* setTrain - устанавливает заданный поезд в заданное место карты. Это тоже выполняется при запуске игры
...
Позиционные триггеры проверяются по наступлении события "изменение состояния траектории". То есть когда где-либо на карте происходит либо занятие, либо освобождение траектории. Данные триггеры могут удаляться после срабатывания, а могут оставаться в системе и срабатывать снова и снова. Существует доступный разработчику сценария механизм управления временем жизни триггера.
...
Теперь нашу action-функцию мы можем передать в триггер, например во временной
...
Разница в том, что в нашей функции мы можем определить произвольные действия, например построить не один, а два маршрута
...
Логика работы нашей action-функций может быть произвольной. К сожалению, для полного овладения всеми возможностями, которые предоставляет данный механизм, придется погрузиться в изучение языка Lua. Но некоторые вещи мы все же разберем здесь.

### 4.3. Передача параметров в action-функцию

В предыдущем примере мы сконструировали action-функции без параметров, а в вызываемые внутри системные функции передали конкретные имена траекторий и направление.

Однако, когда создается функция, обычно это подразумевает некоторые универсальные операции, выполняемые многократно.

Расмотрим такую задачку - поставить временной триггер на построение сквозного пропуска в двух направлениях для произвольной станции. Причем action-функция будет одна единественная, а триггеров мы поставим два - один для станции B, другой для станции C.
...
Ставим триггер на время 17:30:20, а в качестве action-функции передаем результат работы функции dual_approach, которая создает action-функцию для триггера с нужными нам параметрами.
...
Такой механизм, когда мы конструируем функцию "на ходу", передаем её для использования другой функции, и при этом она может выполниться не прямо сейчас, а гораздо позже, по наступлению определенного события, носит название "анонимная функция", или лямбда-функция. Такой подход широко используется в скриптовом языке Lua, движок сценариев RRS активно его эксплуатирует.

### 4.4. Триггеры, устанавливающие триггеры, устанавливающие триггеры...

С помощью action-функций можно добиться того, что, например, установленный нами триггер на одно событие может сам установить триггер уже на другое событие. Это позволяет создать в сценарии цепочку взаимосвязанных друг с другом действий, не обязательно четко привязанных к конкретному времени.
...
Первым вариантом решения такой задачи могло бы быть создание временны́х триггеров, которые отработают последовательно на построение маршрутов. Но кто дает гарантию, что мы угадаем моменты времени, когда следует строить тот или другой маршрут? Разумеется, поезда должны следовать по графику, и определенные временны́е рамки всех перечисленных операций у нас имеются. Но мы не можем и не должны полагаться на это! В реальной жизни, дежурный по станции прежде всего готовит маршруты поезда, исходя из свободности и занятости участков станции, а соблюдение графика складывается в целом из его грамотных действий и грамотной работы машинистов поездов.
...
* traj_name - имя траектории, которая была занята или освобождена
...
if traj_state.in_route then
	...
	-- Действия, которые мы предпримем, если траектория включена в маршрут
...
Следующая функция может быть полезна при отладке сценариев. Эта функция выводит произвольное текстовое сообщения в лог симулятора logs/journal.log

```
logMessage("Hello from RRS scenario system!")
```

Это может помочь разработчику понять, какие части его сценария выполняются и в какое время. Данные сообщения содержат строку "LUA DEBUG:", которая поможет вам отделить их от достаточно большого количества информации, выводимой в лог при работе игры.

### 4.7. Практическое применение функции позиционного триггера

Представим себе ситуацию, когда мы ставим триггер на занятие второго участка приближения, расчитывая построить маршрут пропуска для поезда. Хорошо, если все необходимые участки станции будут при этом свободны. Но может ли оказаться иначе? Разумеется. Например, горловина станции может быть занята другим зазевавшимся поездом. Или участки, через которые будет пролегать маршрут пропуска, будут включены в другой маршрут, например маршрут отправления другого поезда.

Разумеется, если движение в сценарии организуется по графику, такие ситуации могут и не возникнуть, но гарантировать этого нельзя. Покажем, как средствами сценария можно подобную историю грамотно обработать.

Подготовим новый cценарий для всего этого безобразия
...
Поезд 104 находится рядом со вторым участком приближения. Но мы бездумно построим маршрут отправления поезду 102, который стоит на 4 пути станции

```
setTimeTrigger("+00:00:36", actionBuildTrainRoute(train102.traj, stB_CHU1, train102.dir))
```

Пока поезд 104 доедет до второго приближения, маршрут уже будет построен. Соответственно горловина станции В будет занята, и сквозной маршрут этому поезду построить не выйдет. Однако, создав проблему, мы героически решим её с помощью такой функции для позиционного триггера
...
function train104_appr(train_name, traj_name, is_busy)
	...
	if is_busy and traj_name == stB_CHP2 then
		...
		if traj_state.is_busy or traj_state.in_route then
			...
			-- Ставим триггер на его освобождение с целью таки построить маршрут
		...
		-- В любом случае триггер удаляем, он отработал
		return TRIG_DELETE
...
Триггер срабатывает на условие занятия второго участка приближения
...
Однако, не спешим строить маршрут, для начала проверим состояние первого участка удаления
...
Далее, если он или занят, или участвует в другом маршруте

```
if traj_state.is_busy or traj_state.in_route then
```

мы ставим триггер на его освобождение, и уже этот триггер построит для поезда 104 маршрут пропуска, когда поезд 102 уберется с первого удаления
...
Если же вдруг участок удаления окажется свободен, просто построим маршрут пропуска
...
Отработавший триггер, независимо от занятости траектории удаления, мы приказываем убрать из системы
...
Оставляем триггер только в том случае, если второй участок приближения еще не занимался
...
Не забываем установить нашу триггерную функцию в системе
...
Надо сказать, что данный пример довольно синтетический, однако он иллюстрирует, что функция позиционного триггера может быть написана самостоятельно и содержать довольно развесистую логику.
