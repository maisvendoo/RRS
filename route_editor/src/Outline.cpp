    ...
    : observer_viewer(observer_viewer)
{
    ...
    const vsg::ref_ptr<vsg::ShaderSet> flat_shader = vsg::createFlatShadedShaderSet(options);
    const vsg::ref_ptr<vsg::ShaderSet> pbr_shader = vsg::createPhysicsBasedRenderingShaderSet(options);
    const vsg::ref_ptr<vsg::ShaderSet> phong_shader = vsg::createPhongShaderSet(options);
    ...
    const auto frag_shader = read_shader(VK_SHADER_STAGE_FRAGMENT_BIT, shaders_dir.c_str(), "outline.frag", options);
    ...
    const vsg::ref_ptr<vsg::ColorBlendState> color_blend_state = vsg::ColorBlendState::create();
    color_blend_state->attachments = {color_blend_attachment};

    const vsg::ref_ptr<vsg::DepthStencilState> depth_stencil_state = vsg::DepthStencilState::create();
    depth_stencil_state->depthTestEnable = VK_TRUE;
    depth_stencil_state->depthWriteEnable = VK_FALSE;

    const vsg::ref_ptr<vsg::RasterizationState> rasterization_state = vsg::RasterizationState::create();
...
{
    const vsg::ref_ptr<vsg::Node> wireframe_outline = vsg::read_cast<vsg::Node>(paged_lod->filename, options);

    const vsg::ref_ptr<vsg::Viewer> viewer = observer_viewer;
    vsg::CompileResult compile_result = viewer->compileManager->compile(wireframe_outline);
    this->children = {wireframe_outline};
    ...
    const vsg::ref_ptr<vsg::Node> box_outline = builder.createBox(geometry_info, state_info);
