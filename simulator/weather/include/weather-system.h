...
//
//      Дополнительно: суточный ход температуры,
...
/// Зона локального тумана: участок маршрута с пониженной
...
public:
    ...
    /// [Visibility], [FogZone] (локальный туман),
    /// [HeightFog] (высотный туман)
    ...
    /// хода температуры. Вызывается потребителем (моделью);
    ...
    /// Видимость в точке маршрута, м: глобальная с учётом
    ...
    /// Плотность тумана на высоте, 1/м: ниже высоты H0
...
private:
    ...
    /// Дефолты - по, переопределяются секцией [Visibility] weather.conf
