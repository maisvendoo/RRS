...
ExitSignal::ExitSignal(QObject *parent) : StationSignal(parent)
{
    ...
    semaphore_signal_relay->setInitPlusContactState(SS_PLUS_GREEN, false);
    semaphore_signal_relay->setInitMinusContactState(SS_MINUS_YELLOW, false);
...
void ExitSignal::lens_control()
{
    lens_state[RED_LENS] = signal_relay_shunt->getContactState(SRS_CLOSED) &&
                           semaphore_signal_relay->getContactState(SS_N_RED);

    lens_state[YELLOW_LENS] = semaphore_signal_relay->getContactState(SS_N_ALLOW) &&
                              (semaphore_signal_relay->getMinusContactState(SS_MINUS_YELLOW) ||
                              (blink_contact && side_signal_relay->getContactState(SSR_YELLOW)));

    lens_state[GREEN_LENS] = semaphore_signal_relay->getContactState(SS_N_ALLOW) &&
                             semaphore_signal_relay->getPlusContactState(SS_PLUS_GREEN) &&
...
{
    bool is_ALSN_RY_ON = semaphore_signal_relay->getContactState(SS_N_RED);

    alsn_RY_relay->setVoltage(U_bat * static_cast<double>(is_ALSN_RY_ON));

    bool is_ALSN_Y_ON = semaphore_signal_relay->getContactState(SS_N_ALLOW) &&
                        semaphore_signal_relay->getMinusContactState(SS_MINUS_YELLOW);

    alsn_Y_relay->setVoltage(U_bat * static_cast<double>(is_ALSN_Y_ON));

    bool is_ALSN_G_ON = semaphore_signal_relay->getContactState(SS_N_ALLOW) &&
                        semaphore_signal_relay->getPlusContactState(SS_PLUS_GREEN);
