...
public:
    ...
    /// абсолютной траекторной координаты path_coord (0 - начало текущей
    /// траектории ПЕ), м. side: 0 - левый рельс, 1 - правый.
    /// Вызывается вертикальной динамикой ПЕ для каждой колёсной пары
    double getRailHeightAt(double path_coord, int side);

    /// Боковое смещение оси пути от неровностей плана линии, м.
    /// Вызывается поперечной динамикой
    double getLateralOffsetAt(double path_coord);

    /// Возвышение наружного рельса в точке абсолютной траекторной
    /// координаты path_coord, мм (Б16). Знак: "+" - левый рельс выше.
    /// Вызывается ПЕ один раз за шаг (передаётся в вертикальную и
    /// поперечную динамику как параметр cant_mm)
    double getCantAt(double path_coord);

    /// Повредить путь под ПЕ (сход): усиливает неровности для
    /// последующих поездов (цепная реакция аварий)
    void damageTrack(double factor);

    /// Накопить тоннаж на путь от прохода этой ПЕ (износ рельсов,
    /// ТЗ "43-47"): передаётся профилю текущей траектории
    void addTonnage(double path_coord, double mass_tonnes, double distance_m);

    /// Имя текущей траектории ПЕ (для точек погрузки/станций)
    QString getCurrentTrajectoryName() const;
