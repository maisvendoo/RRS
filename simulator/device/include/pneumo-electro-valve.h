...
//------------------------------------------------------------------------------
// Пневматический вентиль с электромагнитным приводом
//------------------------------------------------------------------------------
...
public:
    ...
    /// Задать режим работы клапана: true - закрыт, при подаче напряжения открыт;
    /// false - открыт, при подаче напряжения закрыт;
    void setMode(bool normally_closed = true);
...
private:
    ...
    /// Коэффициент к потоку выхода воздуха из оборудования в атмосферу при закрытом клапане
    double K_atm_device = 0.0;

    /// Коэффициент к потоку выхода воздуха из магистрали в атмосферу при закрытом клапане
    double K_atm_pipe = 0.0;

    /// Коэффициент громкости озвучки к потоку разрядки в атмосферу
    double K_sound = 2.0;
    ...
    /// Режим работы клапана: true - закрыт, при подаче напряжения открыт;
    /// false - открыт, при подаче напряжения закрыт;
    bool mode_normally_closed = true;
