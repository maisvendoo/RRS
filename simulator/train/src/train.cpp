...
void Train::slotStep(const simulator_time_t& current_time, const double& integration_time)
{
        ...
        for (auto it = begin; it != end; ++it)
        {
            ...
            if (i == num_step - 1)
            {
                ...
                *(vehicle->getProfilePoint()) = vc.getPosition();
                ...
                // Накопление тоннажа на путь (ТЗ "43-47", износ рельсов):
    ...
    // Система многих единиц: передача команд с задержкой (ТЗ "СМЕ")
...
//------------------------------------------------------------------------------
/// Безопасное состояние команд СМЕ при потере связи (ТЗ #19, п.19):
...
/// пробуждения из L3-заморозки, ТЗ "Оптимизация", п.5
//------------------------------------------------------------------------------
...
void Train::stepMultipleUnit(double dt)
{
    ...
    // Целостность связи: обрыв сцепки между ПЕ группы СМЕ (ТЗ, п.19).
    ...
    for (Vehicle* vehicle : vehicles)
    {
        ...
        // При потере связи - безопасное состояние (ТЗ #19, п.19):
...
bool Train::loadTrain(QString cfg_path, const init_data_t& init_data, int model_vehicles_count)
{
    ...
    if (cfg.load(cfg_path))
    {
        ...
        // Задержка передачи команд СМЕ, с (ТЗ "Система многих единиц")
