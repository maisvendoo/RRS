...
public:
    ...
    /// Сводка продольной динамики состава (диагностика)
    ...
    /// Снимок диагностики для экранов машиниста: данные из реальных физических систем
    ...
    /// Отладочная строка продольных сил по вагонам:
    ...
    /// Табельный номер игрока, закреплённого за поездом (/): связан с ID пользователя на сайте, по нему выполняется
...
private:
    ...
    /// Табельный номер закреплённого игрока
    int tab_number = -1;
    ...
    /// сцепок в журнал. Вызывается раз в шаг модели
    ...
    /// (обрыв сцепки = потеря управления) -
