...
//
//------------------------------------------------------------------------------
void Model::debugProfileCheck()
{
    static const bool enabled = qEnvironmentVariableIsSet("RRS_PROFILE_DEBUG");
    if (!enabled)
        return;
    ...
    for (Train* train : trains)
    {
        ...
        if (traj == nullptr)
        {
            Journal::instance()->warning(QString("PROFILE DEBUG: trajectory %1 not found").arg(traj_name));
            continue;
        }
        ...
        {
            Journal::instance()->warning(QString("PROFILE DEBUG: getProfile failed for train on %1").arg(traj_name));
            continue;
        }

        if (profile.points.empty())
        {
            Journal::instance()->warning(QString("PROFILE DEBUG: empty profile for train on %1").arg(traj_name));
            continue;
        }

        // Точка отсчёта (distance == 0)
        size_t origin_idx = 0;
        for (size_t i = 0; i < profile.points.size(); ++i)
        {
            if (std::abs(profile.points[i].distance) < 1e-9)
            {
                origin_idx = i;
                break;
            }
        ...
        profile_point_t pp = vc->getPosition();

        // Инвариант: высота и пикетаж точки отсчёта совпадают с данными VehicleController
        double dz = std::abs(profile.points[origin_idx].elevation - pp.position.z);
        double drc = std::abs(profile.points[origin_idx].railway_coord - pp.railway_coord);
        bool origin_ok = (dz < 1e-6) && (drc < 1e-6);

        // Структурные проверки: монотонность дистанции, ограничения пределов
        bool monotonic = true;
        bool limits_ok = (profile.backward >= 0.0) && (profile.forward >= 0.0) &&
                         (profile.backward <= backward_m + 1e-6) &&
                         (profile.forward <= forward_m + 1e-6);
        for (size_t i = 1; i < profile.points.size(); ++i)
        {
            if (profile.points[i].distance <= profile.points[i - 1].distance)
            {
                monotonic = false;
                break;
            }
        ...
        Journal::instance()->info(QString("PROFILE train=%1 traj=%2 coord=%3 orient=%4")
                                      .arg(QString(train->getName().c_str()))
                                      .arg(traj_name).arg(coord, 0, 'f', 3).arg(static_cast<int>(orient)));
        Journal::instance()->info(QString("  points=%1 backward=%2 forward=%3 origin=%4 monotonic=%5 limits=%6")
                                      .arg(profile.points.size())
                                      .arg(profile.backward, 0, 'f', 2)
                                      .arg(profile.forward, 0, 'f', 2)
                                      .arg(origin_ok ? "PASS" : "FAIL")
                                      .arg(monotonic ? "PASS" : "FAIL")
                                      .arg(limits_ok ? "PASS" : "FAIL"));
        Journal::instance()->info(QString("  origin: elev=%1 vc_elev=%2 rc=%3 vc_rc=%4 dz=%5 drc=%6")
                                      .arg(profile.points[origin_idx].elevation, 0, 'f', 3)
                                      .arg(pp.position.z, 0, 'f', 3)
                                      .arg(profile.points[origin_idx].railway_coord, 0, 'f', 3)
                                      .arg(pp.railway_coord, 0, 'f', 3)
                                      .arg(dz, 0, 'g', 6).arg(drc, 0, 'g', 6));

        // Первые и последние три точки ломаной
        size_t n = profile.points.size();
        size_t begin = 0;
        size_t end = std::min(n, (size_t)3);
        for (size_t i = begin; i < end; ++i)
        {
            const profile_segment_t& p = profile.points[i];
            Journal::instance()->info(QString("  first[%1]: d=%2 elev=%3 rc=%4 inc=%5")
                                          .arg(i).arg(p.distance, 0, 'f', 2)
        ...
        {
            const profile_segment_t& p = profile.points[i];
            Journal::instance()->info(QString("  last[%1]: d=%2 elev=%3 rc=%4 inc=%5")
                                          .arg(i).arg(p.distance, 0, 'f', 2)
                                          .arg(p.elevation, 0, 'f', 3)
                                          .arg(p.railway_coord, 0, 'f', 3)
                                          .arg(p.inclination, 0, 'f', 2));
