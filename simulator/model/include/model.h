...
public:
    ...
    /// пассажирских вагонов (система / выключена или проводников нет - true)
    bool isTrainDepartureAllowed(size_t train_idx) const;

    /// Доставлено грузов (заказы точек разгрузки), шт
    unsigned getDeliveredCargoCount() const;

    /// Доставлено груза (заказы точек разгрузки), т
    double getDeliveredTonnes() const;

    /// Загрузка сейва сессии: применяется при
...
signals:
    ...
    /// Рассылка клиентам через сетевой поток:
...
private:
    ...
    struct controlled_client_t
    {
        ...
        /// Табельный номер игрока (-1 - неизвестен)
        int tab_number = -1;
    };
    QMap<int, controlled_client_t> controlled_clients;

    /// Табельные номера подключённых клиентов (client_id -> табельный, /): приходят от клиента при подключении
    QMap<int, int> client_tabs;

    /// "Зависший" поезд отключившегося клиента:
    ...
    /// Таймаут ожидания вернувшегося игрока, с
    ...
    struct LoadingPoint
    {
        ...
        QString order_cargo = "";   ///< Заказ: нужный груз
    ...
    /// Кассета регистрации: запись .kr для управляемой
    ...
    /// Поток сетевого ввода/вывода (высший приоритет)
    QThread     *tcp_thread = nullptr;

    /// Автосохранение сессии: снимки
    ...
    /// Сейв, загруженный организатором: применяется при старте
...
private slots:
    ...
    /// Организатор: закрепить поезд за игроком по табельному
