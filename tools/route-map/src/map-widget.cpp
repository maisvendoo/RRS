...
//
//------------------------------------------------------------------------------
void MapWidget::slotSetVehicleAtCenter(int idx)
{
    follow_player = true;
...
void MapWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (follow_player /*&& (follow_player_idx < players_data->current_vehicles.size())*/)
    {
        ...
        int curr =  current_vehicle;// players_data->current_vehicles[follow_player_idx];
                              ...
                              QPointF& cursor_pos, double& distance2, std::int8_t& dir)
{
    ...
    if (sw->trajectories[SW_FWD_PLUS] && sw->trajectories[SW_FWD_MINUS])
    {
        ...
        case IS_BUSY_PLUS:
        {
            ...
            painter.setPen(pen);
            drawSwitchTraj(fwd_other, other_to_fwd, painter, cursor_pos, distance2);

            pen.setColor(color_switch_busy);
            pen.setWidth(switched_width);
            painter.setPen(pen);
            drawSwitchTraj(fwd_traj, traj_to_fwd, painter, cursor_pos, distance2);
            break;
        }
        case IN_ROUTE_MINUS:
        case IN_ROUTE_PLUS:
    ...
    {
        double fwd_distance2 = distance2;
        ...
        case IS_BUSY_PLUS:
        {
            ...
            pen.setWidth(other_width);
            painter.setPen(pen);
            drawSwitchTraj(bwd_other, other_to_fwd, painter, cursor_pos, distance2);

            pen.setColor(color_switch_busy);
            pen.setWidth(switched_width);
            painter.setPen(pen);
            drawSwitchTraj(bwd_traj, traj_to_fwd, painter, cursor_pos, distance2);
            break;
        }
        case IN_ROUTE_MINUS:
        case IN_ROUTE_PLUS:
                               ...
                               QPointF& cursor_pos, double& distance2)
{
    ...
    dir_t traj_dir = (draw_to_fwd) ? FWD : BWD;
                            ...
                            QPointF& cursor_pos, double& dist2_to_nearest_signal)
{
    ...
    for (auto line_signal : signals_data->line_signals)
    {
        if (line_signal)
        {
            ...
            lens_colors.emplace_back(lens[RED_LENS] ? color_lens_red : color_lens_off);
            lens_colors.emplace_back(lens[GREEN_LENS] ? color_lens_green : color_lens_off);
            lens_colors.emplace_back(lens[YELLOW_LENS] ? color_lens_yellow : color_lens_off);
    ...
    for (auto enter_signal : signals_data->enter_signals)
    {
        if (enter_signal)
        {
            ...
            std::vector<QColor> lens_colors;
            lens_colors.emplace_back(lens[WHITE_LENS] ? color_lens_white : color_lens_off);
            lens_colors.emplace_back(lens[BOTTOM_YELLOW_LENS] ? color_lens_yellow : color_lens_off);
            lens_colors.emplace_back(lens[RED_LENS] ? color_lens_red : color_lens_off);
            lens_colors.emplace_back(lens[GREEN_LENS] ? color_lens_green : color_lens_off);
            lens_colors.emplace_back(lens[YELLOW_LENS] ? color_lens_yellow : color_lens_off);
    ...
    for (auto shunt_signal : signals_data->shunt_signals)
    {
        if (shunt_signal)
        {
            ...
            std::vector<QColor> lens_colors;
            lens_colors.emplace_back(lens[BLUE_LENS] ? color_lens_blue : color_lens_off);
            lens_colors.emplace_back(lens[WHITE_LENS] ? color_lens_white : color_lens_off);
