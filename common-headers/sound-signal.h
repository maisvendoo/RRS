...
struct sound_state_t final
{
    ...
    static float createSoundSignal(char state, float volume, float pitch = 1.0f)
    {
        ...
        const float add_play_num = static_cast<float>(state - 1) * 300.0f;
    ...
    sound_state_t soundFromSignal(float signal)
    {
